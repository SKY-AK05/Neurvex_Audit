# Neurves (Neurvex / Orchvate Audit) — Comprehensive Technical Reference Documentation
...
Neurves is built as a highly responsive, secure, and visually appealing web application. It transitions the old Orchvate auditing workflow into a modern, automated pipeline:

| Dimension | Legacy Workflow | Neurves Workflow |
| :--- | :--- | :--- |
...
Neurves enforces a strict Role-Based Access Control (RBAC) model. Access to resources is checked on both the frontend (Vue Router guards and session state) and the backend (API endpoint authorization layers).
...
The Neurves questionnaire comprises 45 fields: 5 demographic questions (Q1–Q4, including designation) and 40 thematic questions (Q5–Q44) divided into 8 sections.
...
The Neurves scoring engine translates questionnaire responses into quantifiable metrics.
...
Neurves generates high-fidelity HTML email reports designed for compatibility across modern and legacy email clients (including Microsoft Outlook, Gmail, Apple Mail).
...
Rather than running heavy rendering libraries on the backend, Neurves uses the **QuickChart API** to generate the radar chart. The scoring engine builds a JSON configuration, URL-encodes it, and embeds it as an image source:
```
...
Neurves is deployed on **Azure Container Apps (ACA)** in a single-container architecture.

### 11.1 The Single-Container Design
Rather than running separate frontend and backend containers, Neurves bundles both into a single Docker image:
...
Neurves uses a relational database schema designed for PostgreSQL.
...
Neurves implements a multi-layered security model to protect organizational data.
...
Given that Neurves evaluates neurodiversity inclusion, its user interface is built to be accessible, clear, and easy to use.
