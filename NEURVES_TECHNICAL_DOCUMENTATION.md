# Neurvex (Orchvate Audit) — Comprehensive Technical Reference Documentation
...
Several structural factors contribute to the ongoing neglect of neuro-inclusion within standard corporate diversity initiatives.
...
Failure to address these systemic issues results in significant detriments to both the workforce and the organization's bottom line.
...
Neurvex is built as a highly responsive, secure, and visually appealing web application. It transitions the old Orchvate auditing workflow into a modern, automated pipeline. 

The following table contrasts the legacy process with the new Neurvex implementation.

| Dimension | Legacy Workflow | Neurvex Workflow |
| :--- | :--- | :--- |
...
Neurvex enforces a strict Role-Based Access Control (RBAC) model. Access to resources is verified at multiple architectural layers. Frontend protections utilize Vue Router navigation guards, while backend protections rely on API endpoint authorization checks.

> [!IMPORTANT]
> All administrative routes require valid Microsoft Entra ID (SSO) authentication tokens.
...
*   **Access:** Authenticated access via Microsoft Entra ID (SSO) with the `super` role flag.
*   **Permissions:**
...
The data lifecycle within Neurvex moves from an anonymous public client to secure processing, administrator review, and automated delivery.
...
flowchart TD
    ...
    subgraph Admin ["Admin Dashboard"]
        D[Admin Reviews & Edits]
    end
    
    subgraph ACS ["Azure Communication Services"]
        E[ACS Delivery Engine]
    end
    
    subgraph Respondent ["Respondent"]
        F[Receives HTML Report]
    end

    A -- "POST /submit" --> B
    B -- "Inserts Record (status='pending')" --> C
    B -. "Email Alert" .-> Admin
    C -- "Fetches list" --> Admin
    Admin -- "POST /{id}/send" --> E
    E -- "Delivers Email" --> F
    E -- "Updates Record (status='sent')" --> C
```

### 5.2 Step-by-Step Walkthrough

#### 5.2.1 Step 1: Landing and Demographics Collection
...
*   *Validation:* The platform enforces corporate domain validation. Free email providers (e.g., Gmail, Yahoo, Hotmail) are blocked to maintain B2B integrity.

#### 5.2.2 Step 2: Answering the Audit Sections
...
#### 5.2.3 Step 3: Form Submission and Processing
...
#### 5.2.4 Step 4: Administrator Review
...
#### 5.2.5 Step 5: Customization and Dispatch
...
*   The backend receives a `POST` request to `/api/submissions/{id}/send`, converts basic HTML elements to plain text (for multi-part email delivery fallback), routes the dispatch through Azure Communication Services, and updates the database record status to `sent` along with the timestamp.

---

## 6. Audit Structure and Question Analysis

The Neurvex questionnaire is the core diagnostic engine of the platform. It comprises 45 fields: 5 demographic questions (Q1–Q4, including designation) and 40 thematic questions (Q5–Q44) divided evenly into 8 sections.
...
*   **Q6: Explicit inclusion parameters in strategic goals & values.**
...
*   **Q41: Suppliers expected to meet inclusion standards in contracts.**
    *   *Rationale:* Clear contract standards establish inclusion as a shared priority.
...
The Neurvex scoring engine translates qualitative questionnaire responses into precise, quantifiable metrics to determine organizational maturity.
...
The system rounds this score to two decimal places, resulting in a value between 0.00 and 20.00.
...
#### 7.4.1 Underlying Database & Rules Engine Scale
...
#### 7.4.2 High-Fidelity Front-Facing Presentation Scale
...
#### 7.4.3 Detailed Breakdown of the 5 Maturity Levels
...
Neurvex generates high-fidelity HTML email reports designed for compatibility across modern and legacy email clients, including Microsoft Outlook, Gmail, and Apple Mail.

### 8.1 Visual Components

The HTML reports incorporate dynamically rendered visual components to summarize the respondent's data.

#### 8.1.1 Radial Metric Score Arc
...
#### 8.1.2 Dynamic Segment Progress Bars
Within each segment card, a horizontal progress bar displays the section score out of 20 using standard HTML tables to ensure maximum email client compatibility:
...
#### 8.1.3 Interactive Maturity Scale Indicator
...
Rather than running heavy rendering libraries on the backend, Neurvex uses the **QuickChart API** to generate the radar chart. The scoring engine builds a JSON configuration, URL-encodes it, and embeds it as an image source.

```http
GET https://quickchart.io/chart?w=500&h=340&v=2&c=%7B%22type%22%3A%22radar%22...
```
...
*   **Colors:** Accent fill of `rgba(127, 119, 221, 0.15)` with borders and grid markers in Neurvex purple (`#7F77DD`).
...
The administrative dashboard provides a centralized management interface for authorized auditors to oversee the assessment pipeline.
...
The technological foundation of Neurvex is designed for scalability, security, and developer velocity.

### 10.1 Stack Architecture

```mermaid
flowchart TD
    subgraph Frontend ["Frontend (Vue 3 Single Page App)"]
        V[Vue Router 4]
        VT[Vite Build Tool]
        M[Microsoft MSAL SSO]
    end
    
    subgraph Backend ["Backend API (FastAPI / Python)"]
        U[Uvicorn Server]
        P[Psycopg2 DB Driver]
        A[Azure SDK ACS]
    end
    ...
    Frontend -- "HTTP / HTTPS Requests" --> Backend
    Backend -- "SQL Queries (TCP 5432)" --> Database
```

### 10.2 Frontend Architecture (Vue 3)
...
### 10.3 Backend Architecture (FastAPI)
...
### 10.4 Database Layer (PostgreSQL)
...
### 10.5 Authentication Layer (Microsoft Entra ID)
...
Neurvex is deployed on **Azure Container Apps (ACA)** utilizing a highly efficient single-container architecture to reduce overhead.

### 11.1 The Single-Container Design

Rather than running separate frontend and backend containers, Neurvex bundles both into a single Docker image:
...
The core routing logic within the Nginx proxy layer.
...
*   **Secrets Management:** Sensitive variables (e.g., PostgreSQL credentials, ACS connection strings) are stored securely as container secrets in the Azure environment.

---

## 12. System Data Flow Diagrams

This section details the step-by-step transaction pipelines for the two primary system operations.
...
The following endpoints outline the core communication interfaces for the backend service.

> [!WARNING]
> All endpoints expect `Content-Type: application/json` unless otherwise specified.
...
Neurvex utilizes a normalized relational database schema executed in Azure Database for PostgreSQL.
...
CREATE TABLE app_settings (
    ...
    sender_name             VARCHAR(255) NOT NULL DEFAULT 'Neurvex',
    sender_address          VARCHAR(255) NOT NULL DEFAULT '',
    notification_email      TEXT NOT NULL DEFAULT '',
    notification_cc_email   TEXT NOT NULL DEFAULT '',
    notifications_enabled   BOOLEAN NOT NULL DEFAULT FALSE,
    support_email           VARCHAR(255) NOT NULL DEFAULT 'support@orchvate.com',
...
To optimize query performance as the database scales, the following indexes are strictly enforced:
...
Neurvex implements a multi-layered security model to protect organizational data.
...
*   **SQL Injection Prevention:** Database queries use parameterized inputs via `psycopg2` to prevent injection attacks.
*   **SSL Enforcements:** Database connections require SSL, encrypting all data in transit.
*   **CORS Policies:** Cross-Origin Resource Sharing (CORS) configurations restrict API access to trusted frontend origins during development and enforce strict same-origin policies in production environments.

---

## 16. Accessibility (a11y) and UX Design System

Given that Neurvex evaluates neurodiversity inclusion, its user interface is built to be fundamentally accessible, clear, and easy to use.

### 16.1 Design Aesthetics

*   **Harmonious Color Palettes:** The interface uses high-contrast, soft color palettes (Neurvex Purples, Slate Greys, and Lime Green accents) that are visually distinct without causing sensory overwhelm.
*   **Typography:** The platform uses clean, modern sans-serif and serif fonts (**DM Sans** and **Playfair Display**) to ensure high readability for dyslexic users.
*   **Clean Layouts:** Strict grid alignments, generous spacing, and clear visual hierarchies help minimize cognitive load.

### 16.2 UI & UX Features

*   **Progress Tracking:** Persistent step indicators and progress bars clarify the user's position in the assessment, reducing uncertainty and anxiety.
*   **Visual Validation:** Clean, non-intrusive error highlights identify missing responses without creating user stress or using harsh red alerting.
*   **Keyboard Navigation:** All button groups, form inputs, and interactive elements are accessible via standard keyboard controls.
*   **Screen Reader Support:** Standard semantic HTML tags, ARIA roles, and descriptive labels ensure compatibility with assistive technologies.
*   **Contextual Panels:** Sidebar tips explain why each section matters, helping users contextualize the audit without needing external documentation.

---

## 17. Glossary

*   **ACA:** Azure Container Apps. The serverless container hosting platform used by Neurvex.
*   **ACS:** Azure Communication Services. The cloud-based email delivery service used to dispatch diagnostic reports.
*   **a11y:** Accessibility (referring to software and design standards).
*   **DEI:** Diversity, Equity, and Inclusion.
*   **MSAL:** Microsoft Authentication Library. Used to facilitate Single Sign-On (SSO) with Microsoft Entra ID.
*   **Neurodiversity:** The concept that cognitive differences (e.g., autism, ADHD, dyslexia) are natural variations in the human brain.
*   **Orchvate:** The parent organization and consulting body behind the Neurvex software product.
...
*   **Vue SPA:** Vue Single Page Application. A web application that interacts with the user by dynamically rewriting the current web page with new data from the web server.
...
The following environment variables are required in the production container for the system to boot successfully:

*   `DATABASE_URL`: Full PostgreSQL connection string (must include `sslmode=require`).
*   `ACS_CONNECTION_STRING`: Primary connection string for Azure Communication Services.
*   `MSAL_CLIENT_ID`: The application (client) ID registered in Microsoft Entra.
*   `MSAL_TENANT_ID`: The directory (tenant) ID registered in Microsoft Entra.
*   `ENV_MODE`: Determines configuration modes (e.g., `production`, `development`).

### B. Deployment Commands

Standard administrative commands for rebuilding and deploying the Neurvex container:

```bash
# Build the unified production image
docker build -t neurvex-app:latest .

# Run the container locally for testing
docker run -p 8080:80 --env-file .env neurvex-app:latest

# Push to Azure Container Registry
az acr login --name neurvexregistry
docker tag neurvex-app:latest neurvexregistry.azurecr.io/neurvex-app:v2.1
docker push neurvexregistry.azurecr.io/neurvex-app:v2.1
```
