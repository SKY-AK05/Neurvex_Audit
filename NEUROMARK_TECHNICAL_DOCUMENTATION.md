# NeuroMark (Orchvate Audit) — Comprehensive Technical Reference Documentation
...
This section defines the systemic issues within corporate environments that NeuroMark is built to address, highlighting the gap in existing human capital tools.
...
The following table contrasts the legacy process with the new NeuroMark implementation.
...
NeuroMark enforces a strict Role-Based Access Control (RBAC) model. Access to resources is verified at multiple architectural layers. Frontend protections utilize Vue Router navigation guards, while backend protections rely on API endpoint authorization checks.
...
The data lifecycle within NeuroMark moves from an anonymous public client to secure processing, administrator review, and automated delivery.
...
The NeuroMark questionnaire is the core diagnostic engine of the platform. It comprises 45 fields: 5 demographic questions (Q1–Q4, including designation) and 40 thematic questions (Q5–Q44) divided evenly into 8 sections.
...
The NeuroMark scoring engine translates qualitative questionnaire responses into precise, quantifiable metrics to determine organizational maturity.
...
NeuroMark generates high-fidelity HTML email reports designed for compatibility across modern and legacy email clients, including Microsoft Outlook, Gmail, and Apple Mail.
...
*   **Colors:** Accent fill of `rgba(127, 119, 221, 0.15)` with borders and grid markers in NeuroMark purple (`#7F77DD`).
...
The technological foundation of NeuroMark is designed for scalability, security, and developer velocity.
...
NeuroMark is deployed on **Azure Container Apps (ACA)** utilizing a highly efficient single-container architecture to reduce overhead.
...
NeuroMark utilizes a normalized relational database schema executed in Azure Database for PostgreSQL.
...
CREATE TABLE app_settings (
    ...
    sender_name             VARCHAR(255) NOT NULL DEFAULT 'NeuroMark',
...
Given that NeuroMark evaluates neurodiversity inclusion, its user interface is built to be fundamentally accessible, clear, and easy to use.

### 16.1 Design Aesthetics

*   **Harmonious Color Palettes:** The interface uses high-contrast, soft color palettes (NeuroMark Purples, Slate Greys, and Lime Green accents) that are visually distinct without causing sensory overwhelm.
...
*   **ACA:** Azure Container Apps. The serverless container hosting platform used by NeuroMark.
...
*   **Orchvate:** The parent organization and consulting body behind the NeuroMark software product.
...
Standard administrative commands for rebuilding and deploying the NeuroMark container:

```bash
# Build the unified production image
docker build -t neuromark-app:latest .

# Run the container locally for testing
docker run -p 8080:80 --env-file .env neuromark-app:latest

# Push to Azure Container Registry
az acr login --name neuromarkregistry
docker tag neuromark-app:latest neuromarkregistry.azurecr.io/neuromark-app:v2.1
docker push neuromarkregistry.azurecr.io/neuromark-app:v2.1
```
